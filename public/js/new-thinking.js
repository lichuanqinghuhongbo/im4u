...
//        this.FocusDesktop = $('#desktop-' + PLATFORM_NUM + '-1');//默认设置每个平台界面的第一个桌面为焦点
    ...
    function Desktop(root, options, index) {
        ...
        //保存到根上plat.obj.desktop1
        root['desktop' + index] = this;
        //保存每个桌面的对象	
        ...
        //可以自由向桌面添加层
        this.addHtmlToDesktop = function (html) {
            desktop.append(html);
            return self;
        }
    ...
    function aerowindowPlugin(root,dtop, options) {
        var self = this,
            ...
            defaults = {
                ...
                content: '',//窗体内容
                container:'',//透明容器内容，与content只能二选一
                draggable: !0,//拖动窗体
        ...
        if (opts.draggable) {
            winobj.draggable({
                ...
                cursor: 'move',
                start: function (e) {
                    if (opts.resizable && (opts.status == 'maximized' || opts.status == 'minimized')) {
                ...
                stop: function (e) {
                    //opts.top = winobj.offset().top;
                    //opts.left = winobj.offset().left;
                    if (e.pageY <= 0 && opts.btns.maximize) {
        ...
        if (opts.desktopIcon.show) {
            var iconopt = {
                ...
                createWindow: !1,
                iconid: 'desktopicon-' + opts.wid.split('-')[1]
            }
        ...
        function setBtnCss(o) {
            $.each(o, function (i, v) {
                ...
                //console.log(key,bl);
                opts.btns[key] ? (ob.css('display', bl ? 'block' : 'none')) : !!0;
            });
        }
        ...
        this.resize = function (status) {
            ...
            switch (status) {
                case 'regular':
                    ...
                    winobj.draggable({cursorAt: null});
                    opts.resizable && winobj.resizable('enable');
                    break;
                ...
                case 'minimize':
                    ...
                    winobj.animate({
                            ...
                            top: -100 + $('#taskbar-' + opts.wid).offset().top,
                            left: $('#taskbar-' + opts.wid).offset().left / 2
                        },
                        {
                            queue: true,
                    ...
                    opts.resizable && winobj.resizable('disable');
...
//				else{winobj.animate({opacity:'fast'},effect);}
                    ...
                    container.stop().animate({'opacity': 1,
                        ...
                        complete: function () {
                            opts.resizable && winobj.resizable({disabled: true});
                ...
                case  'close' :
                    ...
                    $('#taskbar-' + opts.wid).css({display: 'none'});
                    opts.status = 'closed';
                    break;
        ...
        this.dblclickDesktopIcon = function(e)
        {
            if (opts.status == 'minimized') {
                ...
                self.resize('changeSize');
                //$('#taskbar-' + opts.wid).clone(true).appendTo("ul.mindesktopicons");
                container.css('visibility', 'visible');
                $('#taskbar-' + opts.wid).show();
            }
        }
        ...
        winobj.dblclick(function () {
            if(!opts.resizable) return false;
            if (opts.btns.maximize && opts.status == 'regulared') {
                self.resize('maximize');
                return;
            }
            switch (opts.status) {
        ...
        this.taskbarHandler = function () {
            if (settings.taskbar.show && opts.taskbar.showIcon) {
                ...
                var taskbaritem, minicons, itemid = 'li#taskbar-' + opts.wid;
                if (opts.taskbar.position == 'left') {
                    minicons = $('.taskbar-left ul', top);
                    minicons.append('<li id="taskbar-' + opts.wid + '" class="taskbar-item" title="' + opts.title + '"><img src="' + opts.taskbar.icon + '"></li>');
                }
                else {
                    minicons = $('.taskbar-right ul', top);
                    minicons.append('<li id="taskbar-' + opts.wid + '" class="taskbar-item" title="' + opts.title + '"><img src="' + opts.taskbar.icon + '"></li>');
                }
